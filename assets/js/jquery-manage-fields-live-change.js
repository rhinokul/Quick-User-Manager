...
function qum_hide_properties_for_already_added_fields( container_name ){
    ...
    jQuery( container_name + ' ' + '.element_type_default-e-mail .mbdelete,' + ' ' + container_name + ' ' + '.element_type_default-password .mbdelete,' + ' ' + container_name + ' ' + '.element_type_default-username .mbdelete'  ).hide();	// qum specific line
}



function qum_hide_all ( container_name ){
	...
	jQuery( container_name + ' ' + '.element_type_default-e-mail .mbdelete,' + ' ' + container_name + ' ' + '.element_type_default-password .mbdelete,' + ' ' + container_name + ' ' + '.element_type_default-username .mbdelete'  ).hide();	// qum specific line
	jQuery( container_name + ' ' + '.element_type_default-e-mail #field' + ', ' + container_name + ' ' + '.element_type_default-password #field' + ',  ' + container_name + ' ' + '.element_type_default-username #field' + ', ' + container_name + ' ' + '.element_type_default-e-mail #required' + ', ' + container_name + ' ' + '.element_type_default-password #required,'  + container_name + ' ' + '.element_type_default-username #required,'  + container_name + ' ' + '.element_type_checkbox-terms-and-conditions #required,'  + container_name + ' ' + '.element_type_recaptcha #required' ).attr( 'disabled', true );		// qum specific line
}


function qum_disable_add_entry_button( container_name ){
...
function qum_edit_form_properties( container_name, element_id ){
	qum_hide_all ( container_name );
	qum_disable_add_entry_button ( container_name );
	...
	if ( ( field in fields ) ){
        ...
        qum_handle_user_role_field( container_name );
	}
}


function qum_display_needed_fields( index, container_name, current_field_select ){
	...
	if ( ( ( typeof properties !== 'undefined' ) && ( properties ) ) ) { //the extra (second) condition is a particular case since only the username is defined in our global array that has no meta-name
        ...
        if( jQuery( current_field_select).parents('.update_container_qum_manage_fields').length != 0 ){
            meta_value = jQuery( '.row-meta-name pre', jQuery( current_field_select).parents( '.update_container_qum_manage_fields' ).prev() ).text();
        }
        /* for the add form it should change */
        else{
            numbers = new Array();
            jQuery( '#container_qum_manage_fields .row-meta-name pre').each(function(){
...
*
 */
function qum_handle_user_role_field( container_name ) {
...
function qum_initialize_live_select( container_name ){	
	qum_hide_all( container_name );
	jQuery(document).on( 'change', container_name + ' ' + '.mb-list-entry-fields #field', function () {
		...
		if ( field != ''){
			qum_hide_all( container_name );
			qum_display_needed_fields( field, container_name, this );
		}else{
			qum_hide_all( container_name );
		}
	});
...
jQuery(function(){
 	qum_initialize_live_select ( '#qum_manage_fields' );
	qum_initialize_live_select ( '#container_qum_manage_fields' );
	
	qum_hide_properties_for_already_added_fields( '#container_qum_manage_fields' );
	qum_disable_add_entry_button ( '#qum_manage_fields' );
});