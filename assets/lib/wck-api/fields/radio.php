...
if( !empty( $details['options'] ) ){
	...
	foreach( $details['options'] as $option ){
		...
		$element .= '<div><label><input type="radio" name="'. $single_prefix . esc_attr( Wordpress_Creation_Kit_QUM::wck_generate_slug( $details['title'], $details ) ) .'" id="';
		...
		$element .= esc_attr( Wordpress_Creation_Kit_QUM::wck_generate_slug( $slug_from . '_' . $value_attr ) ) .'" value="'. esc_attr( $value_attr ) .'"  '. checked( $found, true, false ) .'class="mb-radio mb-field" />'. esc_html( $label ) .'</label></div>';
	}
	$element .= '</div>';
}
?>