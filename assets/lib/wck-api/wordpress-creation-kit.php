...
class Wordpress_Creation_Kit_QUM{
	...
	function wck_add_metabox() {
		...
		global $qum_wck_pages_hooknames;
		...
		if( $this->args['context'] == 'post_meta' ){
            ...
            if( !empty( $qum_wck_pages_hooknames[$this->args['post_type']] ) ) {
                add_meta_box($this->args['metabox_id'], $this->args['metabox_title'], array(&$this, 'wck_content'), $qum_wck_pages_hooknames[$this->args['post_type']], $this->args['mb_context'], 'high', array('meta_name' => $this->args['meta_name'], 'meta_array' => $this->args['meta_array']));
                /* add class to meta box */
                add_filter("postbox_classes_" . $qum_wck_pages_hooknames[$this->args['post_type']] . "_" . $this->args['metabox_id'], array(&$this, 'wck_add_metabox_classes'));
            }
		}
	}	
			...
			<ul class="mb-list-entry-fields">
				...
				if( !empty( $fields ) ){
					foreach( $fields as $details ){
                        ...
                        if( $this->args['single'] == true ) {
                            $value = null;
                            if (isset($results[0][Wordpress_Creation_Kit_QUM::wck_generate_slug($details['title'], $details )]))
                                $value = $results[0][Wordpress_Creation_Kit_QUM::wck_generate_slug($details['title'], $details )];
                        }
                        ?>
							<li class="row-<?php echo esc_attr( Wordpress_Creation_Kit_QUM::wck_generate_slug( $details['title'], $details ) ) ?>">
		...
		if($results != null){
			...
			if( !empty( $fields ) ){
				foreach( $fields as $field ){				
					$details = $field;
					if( !empty( $results[$element_id][Wordpress_Creation_Kit_QUM::wck_generate_slug( $details['title'], $details )] ) )
						$value = $results[$element_id][Wordpress_Creation_Kit_QUM::wck_generate_slug( $details['title'], $details )];
					else 
						$value = '';

					$form = apply_filters( "wck_before_update_form_{$meta}_element_{$i}", $form, $element_id, $value );
					
					$form .= '<li class="row-'. esc_attr( Wordpress_Creation_Kit_QUM::wck_generate_slug( $details['title'], $details ) ) .'">';
			...
			$form .= '<a href="javascript:void(0)" class="button-primary" onclick=\'updateMeta("'.esc_js($meta).'", "'.esc_js($id).'", "'.esc_js($element_id).'", "'.esc_js($update_nonce).'")\'><span>'. __( apply_filters( 'wck_save_changes_button', 'Save Changes', $meta ), 'quickusermanager' ) .'</span></a>';
			$form .= '<a href="javascript:void(0)" class="button-secondary" style="margin-left:10px;" onclick=\'removeUpdateForm("'. esc_js( 'update_container_'.$meta.'_'.$element_id ). '" )\'><span>'. __( apply_filters( 'wck_cancel_button', 'Cancel', $meta ), 'quickusermanager' ) .'</span></a>';
			$form .= '</li>';			
	...
	function wck_output_meta_content($meta, $id, $fields, $box_args = '' ){	
		...
		if( !empty( $results ) ){
			$list .= apply_filters( 'wck_metabox_content_header_'.$meta , '<thead><tr><th class="wck-number">#</th><th class="wck-content">'. __( 'Content', 'quickusermanager' ) .'</th><th class="wck-edit">'. __( 'Edit', 'wck' ) .'</th><th class="wck-delete">'. __( 'Delete', 'wck' ) .'</th></tr></thead>' );
			$i=0;
	...
	function wck_output_entry_content( $meta, $id, $fields, $results, $element_id ){
		...
		if( !empty( $fields ) ){
			foreach( $fields as $field ){
				...
				$list .= '<li class="row-'. esc_attr( Wordpress_Creation_Kit_QUM::wck_generate_slug( $details['title'], $details ) ) .'" data-type="'.$details['type'].'"><strong>'.$details['title'].': </strong>'.$display_value.' </li>' . "\r\n";
				...
				if ($meta == 'wck_cfc_fields') {
					if( !empty( $results[$element_id][Wordpress_Creation_Kit_QUM::wck_generate_slug( $details['title'], $details )] ) ){
						$field_title = $results[$element_id][Wordpress_Creation_Kit_QUM::wck_generate_slug( $details['title'], $details )];
		...
		$list .= '</td>';				
		$list .= '<td style="text-align:center;vertical-align:middle;" class="wck-edit"><a href="javascript:void(0)" class="button-secondary"  onclick=\'showUpdateFormMeta("'.esc_js($meta).'", "'.esc_js($id).'", "'.esc_js($element_id).'", "'.esc_js($edit_nonce).'")\' title="'. __( 'Edit this item', 'quickusermanager' ) .'">'. apply_filters( 'wck_edit_button', __('Edit','wck'), $meta ) .'</a></td>';
		$list .= '<td style="text-align:center;vertical-align:middle;" class="wck-delete"><a href="javascript:void(0)" class="mbdelete" onclick=\'removeMeta("'.esc_js($meta).'", "'.esc_js($id).'", "'.esc_js($element_id).'", "'.esc_js($delete_nonce).'")\' title="'. __( 'Delete this item', 'quickusermanager' ) .'">'. apply_filters( 'wck_delete_button', __( 'Delete', 'wck' ), $meta) .'</a></td>';
	...
	function wck_print_scripts($hook){
		...
		if( $this->args['context'] == 'post_meta' ) {		
			if( 'post.php' == $hook || 'post-new.php' == $hook){
				...
				/* only add on Quick User Manager custom post types */
				if( !empty( $_GET['post'] ) )
					$post_id = $_GET['post'];
				else if( !empty( $_POST['post_ID'] ) )
					$post_id = $_POST['post_ID'];
				...
				if( !empty( $post_id ) ){
					...
					if( strpos( $current_post_type, 'qum-' ) === false )					
						return '';
				}			
		...
		elseif( $this->args['context'] == 'option' ){
			if( $qum_wck_pages_hooknames[$this->args['post_type']] == $hook ){				
...
//		wp_enqueue_script( 'wck-tinymce-init' );
		...
		if( !empty( $fields ) ){
			foreach( $fields as $field ){
				if( !empty( $field['required'] ) && $field['required'] )
					$required_fields[Wordpress_Creation_Kit_QUM::wck_generate_slug( $field['title'], $field )] = $field['title'];
			}
		}
	...
	function wck_add_sync_translation_metabox(){
		...
		if( isset( $_GET['lang'] ) ){
			...
			if($has_wck_with_unserialize_fields){
				add_meta_box( 'wck_sync_translation', __( 'Syncronize WCK', 'quickusermanager' ), array( &$this, 'wck_add_sync_box' ), $post->post_type, 'side', 'low' );
			}
	...
	function wck_add_sync_box(){
		global $post;
		?>	
		<span id="wck_sync" class="button" onclick="wckSyncTranslation(<?php echo $post->ID; ?>)"><?php _e( 'Syncronize WCK Translation', 'quickusermanager' ) ?></span>
		<?php 
	}
...
class WCK_Page_Creator_QUM{
	...
	function wck_page_init(){			
		...
		if( $this->args['page_type'] == 'menu_page' ){
			...
			$qum_wck_pages_hooknames[$this->args['menu_slug']] = $this->hookname;
		}
		...
		do_action( 'WCK_Page_Creator_QUM_after_init', $this->hookname );
