 ...
 */
function qum_check_for_unapproved_user( $data, $what ){
	$retMessage = '';
	$messageNo = '';
	...
	if( !empty( $qum_generalSettings['adminApproval'] ) && $qum_generalSettings['adminApproval'] == 'yes' ){
		...
		if ( wp_get_object_terms( $user->data->ID, 'user_status' ) ){
			$retMessage = '<strong>'. __('ERROR', 'quickusermanager') . '</strong>: ' . __('Your account has to be confirmed by an administrator before you can use the "Password Reset" feature.', 'quickusermanager');
			$retMessage = apply_filters('qum_recover_password_unapporved_user', $retMessage);
 ...
 *
 */
function qum_retrieve_activation_key( $requested_user_login ){
	global $wpdb;
	...
	if ( empty( $key ) ) {
		...
		do_action('qum_retrieve_password_key', $requested_user_login, $key);
...
function qum_create_recover_password_form( $user, $post_data ){
	?>
	<form enctype="multipart/form-data" method="post" id="qum-recover-password" class="qum-user-forms" action="<?php echo esc_url( add_query_arg( 'finalAction', 'yes', qum_curpageurl() ) ); ?>">
		<ul>
	<?php
		...
		$recover_inputPassword = '
			<li class="qum-form-field passw1">
				<label for="passw1">'. __( 'Password', 'quickusermanager' ).'</label>
...
?>
		...
		<p class="form-submit">
			<?php $button_name = __('Reset Password', 'quickusermanager'); ?>
			<input name="recover_password2" type="submit" id="qum-recover-password-button" class="submit button" value="<?php echo apply_filters('qum_recover_password_button_name1', $button_name); ?>" />
 ...
 *
 */
 function qum_create_generate_password_form( $post_data ){
	?>
	<form enctype="multipart/form-data" method="post" id="qum-recover-password" class="qum-user-forms" action="<?php echo esc_url( add_query_arg( 'submitted', 'yes', qum_curpageurl() ) ); ?>">
	<?php
	$recover_notification = '<p>' . __( 'Please enter your username or email address.', 'quickusermanager' );
	$recover_notification .= '<br/>'.__( 'You will receive a link to create a new password via email.', 'quickusermanager' ).'</p>';
	echo apply_filters( 'qum_recover_password_message1', $recover_notification );

	$username_email = ( isset( $post_data['username_email'] ) ? $post_data['username_email'] : '' );

	$recover_input = '<ul>
			<li class="qum-form-field qum-username-email">
				<label for="username_email">'.__( 'Username or E-mail', 'quickusermanager' ).'</label>
				<input class="text-input" name="username_email" type="text" id="username_email" value="'.trim( $username_email ).'" />
			</li><!-- .username_email --></ul>';
	echo apply_filters( 'qum_recover_password_generate_password_input', $recover_input, trim( $username_email ) );
		?>
	<p class="form-submit">
		<?php $button_name = __('Get New Password', 'quickusermanager'); ?>
		<input name="recover_password" type="submit" id="qum-recover-password-button" class="submit button" value="<?php echo apply_filters('qum_recover_password_button_name3', $button_name); ?>" />
...
<?php
			...
			if( isset( $_GET['submitted'] ) && isset( $_GET['loginName'] ) && isset( $_GET['key'] ) ){
				...
				if (($messageNo == '') || ($messageNo == '2') || ($messageNo == '4')){
					...
					echo apply_filters( 'qum_recover_password_displayed_message2', '<p class="qum-success">'.$message.'</p>' ); //display success message
			}
