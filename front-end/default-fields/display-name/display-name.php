...
add_filter( 'qum_output_form_field_default-display-name-publicly-as', 'qum_display_name_handler', 10, 6 );


/* handle field validation */
function qum_check_display_name_value( $message, $field, $request_data, $form_location ){
...
add_filter( 'qum_check_form_field_default-display-name-publicly-as', 'qum_check_display_name_value', 10, 4 );


/* handle field save */
function qum_userdata_add_display_name( $userdata, $global_request ){
	if ( isset( $global_request['display_name'] ) )
		$userdata['display_name'] = trim( sanitize_text_field( $global_request['display_name'] ) );
		
	return $userdata;
}
add_filter( 'qum_build_userdata', 'qum_userdata_add_display_name', 10, 2 );