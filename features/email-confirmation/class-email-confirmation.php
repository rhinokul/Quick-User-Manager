...
/*************************** LOAD THE BASE CLASS *******************************
 *******************************************************************************
 * The qum_WP_List_Table class isn't automatically available to plugins, so we need
 * to check if it's available and load it if necessary.
 */
if( !class_exists( 'qum_WP_List_Table' ) ){
    require_once( QUM_PLUGIN_DIR.'/features/class-list-table.php' );
}
 ...
 *******************************************************************************
 * Create a new list table package that extends the core qum_WP_List_Table class.
 * qum_WP_List_Table contains most of the framework for generating the table, but we
 ...
 * 
 */
class wpp_list_unfonfirmed_email_table extends qum_WP_List_Table {
     ...
     * qum_WP_List_Table::single_row_columns()
     * 
    ...
    function column_default($item, $column_name){
        switch($column_name){
            ...
            case 'user-meta':
                ...
                return '<a href="#" data-email="'. $item['email'] .'" onclick="'. esc_attr( 'jQuery(\'<div><pre>'. $user_meta_content .'</pre></div>\').dialog({title:\''. addslashes( __("User Meta", "quickusermanager" ) ) .'\', width: 500 }) ;return false;') .'">'. __( 'show', 'quickusermanager' ) .'</a>';
            default:
     ...
     * 
     * 
     * @see qum_WP_List_Table::::single_row_columns()
     * @param array $item A singular item (one full row's worth of data)
    ...
    function column_username($item){
        ...
        //Build row actions
        $actions = array(
            'delete'    => sprintf( '<a href="javascript:confirmECAction( \'%s\', \'%s\', \'%s\', \'' . addslashes( __( 'delete this user from the _signups table?', 'quickusermanager' ) ) . '\' )">' . __( 'Delete', 'quickusermanager' ) . '</a>', qum_curpageurl(), 'delete', $item['ID'] ),
            'confirm'   => sprintf( '<a href="javascript:confirmECAction( \'%s\', \'%s\', \'%s\', \'' . addslashes( __( 'confirm this email yourself?', 'quickusermanager' ) ) . '\' )">' . __( 'Confirm Email', 'quickusermanager' ) . '</a>', qum_curpageurl(), 'confirm', $item['ID'] ),
            'resend'    => sprintf( '<a href="javascript:confirmECAction( \'%s\', \'%s\', \'%s\', \'' . addslashes( __( 'resend the activation link?', 'quickusermanager' ) ) . '\' )">' . __( 'Resend Activation Email', 'quickusermanager' ) . '</a>', qum_curpageurl(), 'resend', $item['ID'] )
        );
    ...
    function get_columns(){
        $columns = array(
            ...
            'username'     	=> __( 'Username', 'quickusermanager' ),
            'email'    		=> __( 'E-mail', 'quickusermanager' ),
            'registered'  	=> __( 'Registered', 'quickusermanager' ),
            'user-meta'  	=> __( 'User Meta', 'quickusermanager' )
        );
    ...
    function get_bulk_actions() {
        $actions = array(
            'delete'    => __( 'Delete', 'quickusermanager' ),
			'confirm'	=> __( 'Confirm Email', 'quickusermanager' ),
			'resend'	=> __( 'Resend Activation Email', 'quickusermanager' )
        );
	...
	function qum_process_bulk_action_message( $message, $url ){
    ...
    function prepare_items() {
         ...
         */
        $per_page = apply_filters('qum_email_confirmation_user_per_page_number', 20);
        ...
        /**
         * The qum_WP_List_Table class does not handle pagination for us, so we need
 ...
 */
function qum_add_ec_submenu_page() {
	if (is_multisite()){
		add_submenu_page( 'users.php', 'Unconfirmed Email Address', 'Unconfirmed Email Address', 'manage_options', 'unconfirmed_emails', 'qum_unconfirmed_email_address_custom_menu_page' );
		...
		if($qum_generalSettings != 'not_found')
			if(!empty($qum_generalSettings['emailConfirmation']) && ($qum_generalSettings['emailConfirmation'] == 'yes'))
				add_submenu_page( 'users.php', 'Unconfirmed Email Address', 'Unconfirmed Email Address', 'manage_options', 'unconfirmed_emails', 'qum_unconfirmed_email_address_custom_menu_page' );
				remove_submenu_page( 'users.php', 'unconfirmed_emails' ); //hide the page in the admin menu
	}
}
add_action('admin_menu', 'qum_add_ec_submenu_page');
 ...
 */
function qum_unconfirmed_email_address_custom_menu_page(){
    ...
    <div class="wrap">
        ...
        <div class="wrap"><div id="icon-users" class="icon32"></div><h2><?php _e('Users with Unconfirmed Email Address', 'quickusermanager');?></h2></div>
		
		<ul class="subsubsub">
			<li class="all"><a href="users.php"><?php _e('All Users', 'quickusermanager');?></a></li>
		</ul>
